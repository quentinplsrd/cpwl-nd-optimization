...
def rescale_equations(list_equations, slopes, intercepts):
    ...
    for equation in list_equations:
        ...
        list_rescaled_equations.append(
            np.r_[new_coeff_piece, new_intercept_piece])
...
def rescale_polytopes(list_polytopes, slopes, intercepts):
    ...
    for polytope in list_polytopes:
        ...
        new_polytope_intercept = (polytope_intercept 
                                  - polytope_coeff) @ (b_x / a_x).T
...
def rescale_variable_values(variable_values, slopes, intercepts):
    ...
    bPlus = bPlus * slopes[-1] + intercepts[-1] - \
        (aPlus * intercepts[:-1]).sum(axis=1)
    bMinus = bMinus * slopes[-1] - (aMinus * intercepts[:-1]).sum(axis=1)

    error = slopes[-1] * error if "error" in variable_values else None
    zPWL = slopes[-1] * zPWL + \
        intercepts[-1] if "zPWL" in variable_values else None
    # TODO: to verify
    zPlus = slopes[-1] * zPlus + \
        intercepts[-1] if "zPlus" in variable_values else None
