...
def find_plane_equation(points_array):
    ...
    if np.allclose(normal_vector, [0, 0, 0]):
        print(
            "Error: The first three points are collinear (lie on a line). Cannot define a unique plane."
        )
        return None, None, None, None
...
def find_affine_set(data: np.ndarray, max_error: float):
    ...
    z_plus_errors = (
        list_z[:, np.newaxis, :] + error_comb[np.newaxis, :, :]
    )  # (n_simplex, n_error, d+1)
    affine_set = np.einsum("sij,sej->sei", list_inv_matrix, z_plus_errors).reshape(
        -1, d + 1
    )
...
def get_tight_parameters(
    data: np.ndarray, affine_set: np.ndarray, max_slope: Optional[float] = None
) -> dict:
    ...
    if not isinstance(affine_set, np.ndarray):
        raise TypeError(
            f"`affine_set` must be a numpy.ndarray, got {type(affine_set).__name__}."
        )
    ...
    if max_slope is not None:
        if not isinstance(max_slope, (int, float, np.floating)):
            raise TypeError(
                f"`max_slope` must be a float or int, got {type(max_slope).__name__}."
            )
