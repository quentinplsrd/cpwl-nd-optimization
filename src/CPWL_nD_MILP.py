...
def calculate_CPWL_approximation(data: np.ndarray, max_error: float,
    ...
    if constant_value[:-1].any():
        raise ValueError(
            "Invalid data: "
            "at least one input dimension is constant-valued."
        )
    ...
    if big_M_constraint=='tight':
        print("Calculating all combinations of affine functions and the tight parameters")
    ...
    model, variables, result = solve_CPWL_approximation(
        rescaled_data, max_error=rescaled_error, 
        N_plus=N_plus, N_minus=N_minus,
        objective=objective, big_M_constraint=big_M_constraint, 
        integer_feasibility_tolerance=integer_feasibility_tolerance,
        enable_output=enable_output,
        solver=solver, default_big_M=default_big_M, 
    ...
    original_affine_pieces = transform_affine_pieces(affine_pieces, slopes3, intercepts3)
